 ...
 * Adds bilingual support to trips, collections, destinations, journal_articles, trip_stops, and faqs tables
 */
async function addLanguageFields() {
  ...
  try {
    ...
    async function addColumnIfNotExists(tableName, columnDef, afterColumn) {
      ...
      if (exists) {
        return; // Column already exists, skip silently
      }
    ...
    console.log('    ✓ Language fields migration completed');

  } catch (error) {
    console.error('    ❌ Error during language fields migration:', error.message);
    throw error;
  } finally {
